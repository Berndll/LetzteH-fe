...
Beim Kondensator ist der Strom der Spannung 90° voraus:
\begin{center}
\begin{tikzpicture}[>=latex]
    ...
    \draw[->,thick,black] (0,0) -- (posX) node[right,xshift=3pt] {$Re(j\omega)$};
    \draw[->,thick,black] (0,0) -- (posY) node[right,xshift=3pt] {$Im(j\omega)$};
    \draw[->,thick,black] (0,0) -- (negX);
    \draw[->,thick,black] (0,0) -- (negY);

    % Pointer
    \draw[->,thick,blue] (0,0) -- (0:2)  node[above right] {$\underline{U}$};
    \draw[->,thick,red]  (0,0) -- (90:2) node[above right] {$\underline{I}$};

    % Arc
    \draw[->, thick] (posX) arc (0:30:2) node[right] {$\omega$};
\end{tikzpicture}
\end{center}
...
\subsection{Zeigerdiagramm}
Bei der Spule ist der Spannung der Strom 90° voraus:
\begin{center}
\begin{tikzpicture}[>=latex]
    \draw[style=help lines] (0,0) (3,2);
    ...
    % Pointer
    \draw[->,thick,blue] (0,0) -- (90:2) node[above right] {$\underline{U}$};
    \draw[->,thick,red]  (0,0) -- (0:2)  node[above right] {$\underline{I}$};

    % Arc
    \draw[->, thick] (posX) arc (0:30:2) node[right] {$\omega$};
\end{tikzpicture}
\end{center}
\textit{Merksatz:"Spule Spannung vor!"}

\subsection{Blindwiderstand}
Der Blindwiderstand einer Spule wird mit $\underline{x}_L = j\omega L$ beschrieben. Das heißt, die Impedanz (der "Widerstand") ist abhängig von der Frequenz ($j\omega$).

\begin{itemize}
    \item Bei niedrigen Frequenzen wird die Spule zum Kurzschluss:\\
    $\omega \rightarrow 0: j\omega L = 0$

    \item Bei hohen Frequenzen wird der Kondensator zum Kurzschluss:\\
    $\omega \rightarrow \infty: j\omega L = \infty$
\end{itemize}
...
\begin{center}
    \begin{tikzpicture}
        \begin{semilogxaxis}[
            ...
            \addlegendentry{$R_{H}\neq R_{T}$ \hspace{0.3cm} $C_{H}\neq C_{T}$}
            \addlegendentry{$R_{H}=R_{T}$ \hspace{0.3cm} $C_{H}=C_{T}$};
